...
import { AnimatedGLBModel } from "@/components/mmorpg/viewer/AnimatedGLBModel"
...
import { getMovementStateFromInput, getAnimationFromMovementState, type PlayerMovementState } from "@/lib/mmorpg/playerAnimationState"
import { getMovementSpeeds } from "@/lib/mmorpg/mountSpeed"

type WorldMountMode = "on-foot" | "companion" | "mounted"

interface AnimatedPlayerControllerProps {
...
export function AnimatedPlayerController({
  useRealModels, selectedCharacterId, selectedMountId,
  worldMountMode = "companion",
  onPositionChange, onMovementStateChange, useAnimatedPlayer,
  externalPosition,
}: AnimatedPlayerControllerProps) {
  const groupRef = useRef<THREE.Group>(null)
  const [movementState, setMovementState] = useState<PlayerMovementState>("idle")
  const [animClips, setAnimClips] = useState<string[]>([])

  const animPreset = getAnimationPreset(selectedCharacterId)
  const isTicker = selectedCharacterId === "ticker"
  const canAnimate = useAnimatedPlayer && isTicker && animClips.length > 0 && worldMountMode !== "mounted"
  ...
  useFrame((_, delta) => {
    ...
    const state = getMovementStateFromInput({ isMoving, isRunning })
    setMovementState(state)
  })

  useEffect(() => {
    onMovementStateChange?.(movementState)
  }, [movementState, onMovementStateChange])

  const animationName = canAnimate ? getAnimationFromMovementState(movementState) : undefined
    ...
    <group ref={groupRef} position={[0, 0, 0]}>
      {worldMountMode === "mounted" && selectedMountId ? (
        ...
        <>
          {canAnimate ? (
            <AnimatedGLBModel
              url={animPreset!.riggedModelUrl!}
              animationName={animationName}
              position={character?.modelPosition ?? [0, 0, 0]}
              scale={character?.modelScale ?? [1, 1, 1]}
              rotation={character?.modelRotation ?? [0, 0, 0]}
              onAnimationsLoaded={setAnimClips}
            />
