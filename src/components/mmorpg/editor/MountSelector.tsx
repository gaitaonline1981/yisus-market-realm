...
export function MountSelector({ selectedId, onSelect }: Props) {
  return (
    <div>
      <div className="mb-2.5 text-[13px] font-bold uppercase tracking-wider text-zinc-100">Monturas</div>
      <div className="flex flex-col gap-1.5">
        {mounts.map((m) => {
          const selected = m.id === selectedId
          const rarityColor = RARITY_CONFIG[m.rarity]?.color ?? "#F59E0B"
          return (
            ...
            >
              ...
              <div>
                <p className={`text-[11px] font-bold ${selected ? "text-white" : "text-zinc-300"}`}>{m.name}</p>
                <p className="text-[9px] text-zinc-500">{m.type}</p>
              </div>
            </button>
          )
        })}
